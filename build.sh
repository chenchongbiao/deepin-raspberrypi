...
sudo cp -a modules/* $TMP/lib/modules/
...
curl -L -o $TMP/tmp/raspi-config.deb https://archive.raspberrypi.org/debian/pool/main/r/raspi-config/raspi-config_20240521_all.deb
run_command_in_chroot "$TMP" "export DEBIAN_FRONTEND=noninteractive && \
    apt update -y && apt install -y \
    $TMP/tmp/raspi-config.deb"
